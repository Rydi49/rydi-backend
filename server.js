...
async function getBikeWithStats(bike) {
  ...
  // Privacy: don't expose owner email in public API
...
app.post('/api/auth/forgot-password', authLimiter, async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email: email.toLowerCase() });
    if (!user) {
      // Don't leak whether email exists — return same message
    ...
    if (SENDGRID_API_KEY) {
      await emailService.send({
        to: user.email,
        subject: 'RYDI Password Reset',
        text: `Hi ${user.firstName},\n\nYou requested a password reset for your RYDI account.\n\nClick this link to reset your password:\nhttps://rydi.ca/reset-password?token=${resetToken}\n\nThis link expires in 15 minutes.\n\nIf you didn't request this, ignore this email.`,
      });
    }
...
app.post('/api/bookings', protect, resolveUser, async (req, res) => {
  try {
    ...
    if (overlap) {
      return res.status(409).json({ success: false, message: 'This bike is already booked for those dates. Please select different dates.' });
    }

    const days = Math.ceil((new Date(endDate) - new Date(startDate)) / (1000 * 60 * 60 * 24));
    const basePrice = bike.dailyRate * days;

    // Load dynamic platform settings
    const settings = await getPlatformSettings();
    const tierConfig = {
      basic: { name: 'Basic Protection', dailyPrice: 0, deposit: settings.depositBasic },
      standard: { name: 'Standard Protection', dailyPrice: settings.protectionStandardDaily, deposit: settings.depositStandard },
      premium: { name: 'Premium Protection', dailyPrice: settings.protectionPremiumDaily, deposit: settings.depositPremium },
    };
    const tier = tierConfig[protectionTier] || tierConfig.basic;
    const protCost = tier.dailyPrice * days;
    const serviceFee = Math.round(basePrice * (settings.platformFeePercent / 100));
...
app.post('/api/bookings/:id/cancel', protect, resolveUser, async (req, res) => {
  try {
    ...
    if (hoursUntil >= settings.cancellationFullRefundHours) {
      refundPercent = 100; // Full refund
      cancellationFee = 0;
    } else if (hoursUntil >= settings.cancellationPartialRefundHours) {
      refundPercent = settings.cancellationPartialRefundPercent; // Partial refund
      cancellationFee = booking.totalPrice * ((100 - refundPercent) / 100);
    } else {
...
app.get('/api/admin/users', protect, resolveUser, adminOnly, async (req, res) => {
  try {
    ...
    const enriched = users.map(u => {
      const obj = u.toJSON(); // toJSON strips password and __v
      return {
        ...obj,
...
app.delete('/api/admin/users/:id', protect, resolveUser, adminOnly, async (req, res) => {
  try {
    if (!isValidObjectId(req.params.id)) {
      return res.status(400).json({ success: false, message: 'Invalid user ID format' });
    }
    ...
    await Bike.deleteMany({ ownerId: req.params.id });
    await Booking.deleteMany({ renter: req.params.id });
    await Verification.deleteMany({ userId: req.params.id });
...
app.delete('/api/auth/me', protect, resolveUser, async (req, res) => {
  try {
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'OK', timestamp: new Date().toISOString(), version: '3.5.0' });
});
...
app.put('/api/admin/settings', protect, resolveUser, adminOnly, async (req, res) => {
  try {
    const settings = await getPlatformSettings();
    const allowedFields = [
      'platformFeePercent', 'taxRatePercent', 'verificationRequired',
      'manualListingApproval', 'emailNotificationsEnabled', 'lateReturnFeePerHour',
      'cancellationFullRefundHours', 'cancellationPartialRefundHours',
      'cancellationPartialRefundPercent', 'depositBasic', 'depositStandard',
      'depositPremium', 'protectionStandardDaily', 'protectionPremiumDaily'
    ];
    ...
    for (const field of allowedFields) {
      if (req.body[field] !== undefined) {
        // Validate ranges
        if (field === 'platformFeePercent' && (req.body[field] < 0 || req.body[field] > 50)) {
          return res.status(400).json({ success: false, message: 'Platform fee must be 0-50%' });
        }
        if (field === 'taxRatePercent' && (req.body[field] < 0 || req.body[field] > 30)) {
          return res.status(400).json({ success: false, message: 'Tax rate must be 0-30%' });
        }
        if (field.endsWith('Percent') && (req.body[field] < 0 || req.body[field] > 100)) {
          return res.status(400).json({ success: false, message: `${field} must be 0-100%` });
        }
        if (field.endsWith('Daily') || field.startsWith('deposit') || field.endsWith('FeePerHour') || field.endsWith('Hours')) {
          if (req.body[field] < 0) {
...
app.get('/', (req, res) => {
  res.json({
    ...
    version: '3.5.0',
    features: ['Auth', 'Bikes', 'Bookings', 'Reviews', 'Role-based Verifications', 'Owner Dashboard', 'Admin Panel', 'Email Notifications', 'Google Maps', 'Stripe Payments', 'Contact Form', 'Password Reset', 'Dynamic Settings'],
    endpoints: ['/api/auth', '/api/bikes', '/api/bookings', '/api/reviews', '/api/verifications', '/api/owner/stats', '/api/admin', '/api/payments', '/api/contact', '/api/settings']
  });
});
...
connectDB().then(async () => {
  await seedData();
  app.listen(PORT, () => {
    console.log(`RYDI Backend v3.5 (MongoDB + Role-based verification + Security hardening) running on port ${PORT}`);
  });
});
