...
app.use(cors({
  origin: process.env.NODE_ENV === 'production'
    ? ['https://rydi.ca', 'https://www.rydi.ca']
    : ['http://localhost:5173', 'http://localhost:3000', 'https://rydi.ca'],
  credentials: true
}));
app.use(apiLimiter);
...
app.post('/api/verifications', protect, resolveUser, async (req, res) => {
  try {
    ...
    // Max 1MB per base64 image (~1.33MB raw string)
    const MAX_IMAGE_SIZE = 1_500_000;
    if (licenseImage.length > MAX_IMAGE_SIZE) {
      return res.status(413).json({ success: false, message: 'License image exceeds 1MB limit. Please compress or crop.' });
    }
    if (selfieImage.length > MAX_IMAGE_SIZE) {
      return res.status(413).json({ success: false, message: 'Selfie image exceeds 1MB limit. Please compress or crop.' });
    }
    if (icbcImage && icbcImage.length > MAX_IMAGE_SIZE) {
      return res.status(413).json({ success: false, message: 'ICBC card image exceeds 1MB limit. Please compress or crop.' });
    }
...
app.post('/api/payments/create-intent', protect, resolveUser, async (req, res) => {
  try {
    ...
    // CRIT-001 FIX: Require verified ID before allowing payment
    const verification = await Verification.findOne({ userId: req.userDoc._id.toString(), status: 'verified' });
    if (!verification) {
      return res.status(403).json({ success: false, message: 'ID verification required before booking. Please complete identity verification first.' });
    }
...
connectDB().then(async () => {
  await seedData();
  app.listen(PORT, () => {
    console.log(`RYDI Backend v3.1 (MongoDB) running on port ${PORT}`);
  });
});
